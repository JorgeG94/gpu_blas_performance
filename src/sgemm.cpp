...
int main(int argc, char *argv[]) {
  ...
  float *A, *B, *C;
  hipAssert(hipMalloc((void**) &A, m*k*sizeof(float)));
  hipAssert(hipMalloc((void**) &B, k*n*sizeof(float)));
  hipAssert(hipMalloc((void**) &C, m*n*sizeof(float)));
  float *HA, *HB;
  hipAssert(hipHostMalloc((void**)&HA, m*k*sizeof(float)));
  hipAssert(hipHostMalloc((void**) &HB, k*n*sizeof(float)));
	srand(time(NULL));
  for (int i=0; i<m*k; i++)
  ...
  hipblasSetStream(handle,s);
	...
	#endif
  hipAssert(hipMemcpy(A,HA,m*k*sizeof(float),hipMemcpyHostToDevice));
  hipAssert(hipMemcpy(B,HB,n*k*sizeof(float),hipMemcpyHostToDevice));
...
#else
  status = hipblasSgemm(handle, op1, op2, m, n, k,
...
std::cout << std::left << std::setw(12) << "Time (us)" << std::setw(12) << "GFLOP/s" << std::setw(4) << "Rep" << std::endl;
  ...
  auto duration = duration_cast<microseconds>(end-start);
    float gflops = ((float)2 * m * k * n * reps) / duration.count() / 1000;
